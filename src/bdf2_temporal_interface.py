...
# Output paths
# ==========================================================
OUTDIR = Path("bdf2_temporal_interface_outputs")
...
# Raw residual = c1(theta) J1 + h c2(theta) J2 + ...
# ==========================================================
...
def integrate_problem(problem: LinearSwitchProblem, h: float, method: str) -> Tuple[np.ndarray, np.ndarray]:
    if method not in {"raw", "corrected", "restart"}:
        raise ValueError("method must be 'raw', 'corrected', or 'restart'.")
...
def plot_theta_scan(T: float, beta: float, h: float, filename: str) -> None:
    ...
    severe_raw = np.abs([c1(th) * beta for th in thetas])
    benign_raw = np.abs([h * c2(th) * (2.0 * beta) for th in thetas])
    plt.figure(figsize=(7.2, 5.2))
    plt.plot(thetas, severe_raw, label=r"$|c_1(\theta)J_1|$")
    plt.plot(thetas, benign_raw, label=r"$|h c_2(\theta)J_2|$")
...
def main() -> None:
    ...
    benign_studies = compute_error_rows(benign_problem, m_values=range(5, 12), methods=("raw", "corrected"))
    print_table("Benign scalar benchmark: raw", benign_studies["raw"])
    print_table("Benign scalar benchmark: corrected", benign_studies["corrected"])
    save_csv("benign_raw.csv", benign_studies["raw"])
    ...
    severe_studies = compute_error_rows(severe_problem, m_values=range(5, 12), methods=("raw", "corrected"))
    print_table("Severe scalar benchmark: raw", severe_studies["raw"])
    print_table("Severe scalar benchmark: corrected", severe_studies["corrected"])
    save_csv("severe_raw.csv", severe_studies["raw"])
    ...
    aligned_studies = compute_error_rows(aligned_problem, m_values=range(5, 12), methods=("raw", "restart"))
    print_table("Aligned-event experiment: raw", aligned_studies["raw"])
    print_table("Aligned-event experiment: restart", aligned_studies["restart"])
    save_csv("aligned_raw.csv", aligned_studies["raw"])
    save_csv("aligned_restart.csv", aligned_studies["restart"])
    plot_convergence(aligned_studies, "Aligned-event test: raw versus restart", "figure_aligned_restart.png")
    ...
    pde_studies = compute_error_rows(pde_problem, m_values=range(5, 10), methods=("raw", "corrected"))
    print_table("Semidiscrete severe benchmark: raw", pde_studies["raw"])
    print_table("Semidiscrete severe benchmark: corrected", pde_studies["corrected"])
    save_csv("pde_raw.csv", pde_studies["raw"])
