...
for xc, hdr in zip(col_centers, ['RAW BDF2', 'RESTART', 'DIRECT CORRECTION']):
...
rounded_box(xL, y_top, w, h_top,
            'Crossing value at $t_{k+1}$ is produced\nby the raw BDF2 stencil', fontsize=12.8)
...
rounded_box(xM, y_top, w, h_top,
            'Crossing value at $t_{k+1}$ is still produced\nby the same raw BDF2 stencil', fontsize=12.4)
...
rounded_box(xM, y_bot, w, h_bot,
            'Effect on first crossing residual:\nnone\n$R^{\\mathrm{restart}}_{k+1}=R^{\\mathrm{raw}}_{k+1}$', fontsize=12.3)
