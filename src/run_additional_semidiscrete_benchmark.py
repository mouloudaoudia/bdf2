...
2. raw-versus-corrected timing diagnostics;
...
def run_semidiscrete_benign(J: int, m_min: int, m_max: int) -> Dict[str, List[Dict[str, object]]]:
    ...
    studies = base.compute_error_rows(problem, m_values=range(m_min, m_max + 1), methods=("raw", "corrected"))

    base.print_table("Semidiscrete benign benchmark: raw", studies["raw"])
    base.print_table("Semidiscrete benign benchmark: corrected", studies["corrected"])

    _save_csv("pde_benign_raw.csv", studies["raw"])
    ...
    for N in Ns:
        ...
        for method in ("raw", "corrected"):
            ...
            rows.append({
                ...
                "relative_to_raw_median": None,
            })

        raw_med = medians.get("raw")
        if raw_med and raw_med > 0:
            for row in rows:
                if row["N"] == N:
                    row["relative_to_raw_median"] = float(row["median_seconds"]) / raw_med
    ...
    rows = [
        {
            ...
            "recommended_interpretation": "baseline",
        },
        {
            ...
            "effect_on_first_crossing_residual": "does not cancel the mixed crossing residual if raw crossing value has already been generated",
            "recommended_interpretation": "robust history repair, not direct defect cancellation",
        },
        {
            ...
            "recommended_interpretation": "local crossing-defect cancellation when jump data are available",
        },
    ]
